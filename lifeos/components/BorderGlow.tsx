...
function parseHSL(hslStr: string): { h: number; s: number; l: number } {
  // Try to parse an rgb, hex, or anything fallback
  // This is a naive HSL parser for the prompt. We'll fallback to a base pink/purple.
  // We can just rely on the fallback if it's not strictly "h s l" format to be safe.
  const match = hslStr.match(/([\d.]+)\s*([\d.]+)%?\s*([\d.]+)%?/);
  if (!match) return { h: 325, s: 78, l: 58 }; // fallback to #e8437f equivalent
...
const BorderGlow: React.FC<BorderGlowProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="border-glow-inner" style={{ height: "100%" }}>
        {children}
      </div>
    </div>
  );
